  ...
  var hat = {
    ...
    buyvalue: "0,0,0,0,0",
    color1: "white",
    text1: "Buy for 20 Gold",
    color2: "white",
    text2: "Buy for 50 Gold",
    color3: "white",
    text3: "Buy for 75 Gold",
    color4: "white",
    text4: "Buy for 100 Gold",
    color5: "white",
    text5: "Buy for 150 Gold",
    equipedHat:0
  }
  if(user.loggedIn === "true" || user.loggedIn === true) {
    getScore();
    setTimeout(function(){
    sendScore(user.username, Math.floor(user.highscore), user.password, user.mail, Math.floor(coin.value), hat.buyvalue);
    dino.inAir = false;
    dino.died = false;
    user.loggedIn = true;
    }, 2000)
  ...
  window.addEventListener('keydown', function (event) {
    ...
    if (event.code === "ShiftLeft" && user.loggedIn === true || event.code === "KeyS" && user.loggedIn === true || event.code === "ControlLeft" && user.loggedIn === true) {
      crouch()
      dino.fastFall = true
    }
  ...
  function update() {
    ...
    if (dinoImg.complete) {
      ...
      c.drawImage(hatImg, Math.floor(dino.x+70), Math.floor(dino.y-hat.crouchValue), 150, 100);
    ...
    if (coinImg.complete) {
      c.drawImage(coinImg, Math.floor(coin.x), Math.floor(coin.y));
      coinImg.src = coin.image;
      c.fillStyle = "black";
      c.textAlign = "center";
      c.font = "30px IMPACT, Sans-serif";
      c.fillText(Math.floor(coin.value*10)/10, coin.x+48, coin.y+65)
    } else {
    ...
    if (dino.died === false && dino.stand === false && user.loggedIn === true) {
      ...
      if (dino.score < 2500) {
        ...
        dino.speed = Math.floor(2500 / 300 + 20);
      }
  	  if(dino.crouch === true && dino.inAir === false){
        if(hat.crouchValue > -25){
          hat.crouchValue -= 10;
        }
    ...
    c.fillText(dino.timer, 1365/2, 768/4*3);
    if (dino.died === true) {
    ...
    if(menu.shopVisual === true){
      ...
      c.fillStyle = "black"
      c.fillRect(100, 658, 400, 100);
      c.fillStyle = "black"
      c.fillRect(510, 658, 400, 100);
      c.fillStyle = "black"
      c.fillRect(100, 290, 400, 100);
      c.fillStyle = "black"
      c.fillRect(510, 290, 400, 100);
      c.fillStyle = "black"
      c.fillRect(920, 290, 400, 100);
      c.textAlign = "center"
      c.font = "50px IMPACT, Sans-serif";
      c.fillStyle = hat.color1;
      c.fillText(hat.text1, 300, 370)
      c.fillStyle = hat.color2;
      c.fillText(hat.text2, 710, 370)
      c.fillStyle = hat.color3;
      c.fillText(hat.text3, 1120, 370)
      c.fillStyle = hat.color4;
      c.fillText(hat.text4, 300, 740)
      c.fillStyle = hat.color5;
      c.fillText(hat.text5, 710, 740)
      ...
      if(hatArray[4] === "1" && hat.equipedHat !== 5){
        ...
        hat.text5 = "Buy for 150 Gold"
      }

      if(mouse.x > 100 && mouse.x < 500 && mouse.y > 290 && mouse.y < 390){
        hat.color1 = "#A2A2A2";
        if(mouse.click === true){
          if(coin.value >= 20 && hatArray[0] === "0"){
            hatArray[0] = "1";
            hat.buyvalue = hatArray.toString()
            coin.value = coin.value-20;
            sendScore(user.username, Math.floor(user.highscore), user.password, user.mail, Math.floor(coin.value), hat.buyvalue);
            return
          }
          if(hatArray[0] === "1" && hat.equipedHat !== 1){
      ...
      if(mouse.x > 510 && mouse.x < 910 && mouse.y > 290 && mouse.y < 390){
        hat.color2 = "#A2A2A2";
        if(mouse.click === true){
      ...
      if(mouse.x > 920 && mouse.x < 1320 && mouse.y > 290 && mouse.y < 390){
        hat.color3 = "#A2A2A2";
        if(mouse.click === true){
          if(coin.value >= 75 && hatArray[2] === "0"){
            hatArray[2] = "1";
            hat.buyvalue = hatArray.toString()
            coin.value = coin.value-75;
            sendScore(user.username, Math.floor(user.highscore), user.password, user.mail, Math.floor(coin.value), hat.buyvalue);
            return
          }
          if(hatArray[2] === "1" && hat.equipedHat !== 3){
      ...
      if(mouse.x > 100 && mouse.x < 500 && mouse.y > 658 && mouse.y < 758){
        hat.color4 = "#A2A2A2";
        if(mouse.click === true){
          if(coin.value >= 100 && hatArray[3] === "0"){
            hatArray[0] = "1";
            hat.buyvalue = hatArray.toString()
      ...
      if(mouse.x > 510 && mouse.x < 910 && mouse.y > 658 && mouse.y < 758){
        hat.color5 = "#A2A2A2";
        if(mouse.click === true){
          if(coin.value >= 150 && hatArray[4] === "0"){
            hatArray[4] = "1";
            hat.buyvalue = hatArray.toString()
  ...
  function sendScore(username, score, password, mail, coins, hats) {
    const http = new XMLHttpRequest();
    const url = `https://l2niipto9l.execute-api.eu-north-1.amazonaws.com/test/update?username=${username}&score=${score}&password=${password}&mail=${mail}&coins=${coins}&hats=${hats}`;
    http.open("GET", url);
    http.send();
  };
